...
def _eval(args, epoch=None, model=None):
    ...
    device = 'cuda'
    model = Qwen2VLForConditionalGeneration.from_pretrained("Qwen/Qwen2-VL-7B-Instruct", torch_dtype=torch.bfloat16, device_map="auto", trust_remote_code=True, attn_implementation="eager",).eval().to(device)
    ...
    for dd in tqdm(dataloader, desc="Processing", ncols=100):
        ...
        generation_config.pos = inputs['input_ids'].tolist()[0].index(vision_start_token_id) + 1
        generation_config.pos_end = inputs['input_ids'].tolist()[0].index(vision_end_token_id)
