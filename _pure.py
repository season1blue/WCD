...
def data_prepare(model_id: str, processor, image: Image.Image, question: str, vision_start_token_id, vision_end_token_id, device):
    ...
    if "qwen2" == model_id.lower():
        ...
        pos = inputs['input_ids'].tolist()[0].index(vision_start_token_id)
        pos_end = inputs['input_ids'].tolist()[0].index(vision_end_token_id)+1
        ...
        pos = inputs['input_ids'].tolist()[0].index(vision_start_token_id)
        pos_end = pos + 576 + 5 #config中的设置image_seq_length是576
