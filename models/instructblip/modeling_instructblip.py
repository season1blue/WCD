...
from transformers.utils import ModelOutput, TransformersKwargs, auto_docstring, can_return_tuple, logging, torch_int
from transformers.utils.generic import OutputRecorder, check_model_inputs
from transformers.models.auto import AutoModel, AutoModelForCausalLM, AutoModelForSeq2SeqLM
...
class InstructBlipForConditionalGeneration(MyGenerationMixin, InstructBlipPreTrainedModel):
    config: InstructBlipConfig
    main_input_name = "pixel_values"
    ...
    def __init__(self, config: InstructBlipConfig):
        ...
        if config.use_decoder_only_language_model:
            language_model = LlamaForCausalLM._from_config(config.text_config)
        else:
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        image_mask = special_image_mask[0, :, 0]
        true_indices = torch.nonzero(image_mask, as_tuple=False).squeeze()
        if mask_config is not None:
            mask_config.pos = true_indices[0].item()
            mask_config.pos_end = true_indices[-1].item()+1
            outputs = self.language_model.generate(**inputs, **generate_kwargs, mask_config=mask_config)
        else:
