        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if generation_config.output_attentions and generation_config.attn_diff:
                # NEW ADD
                final_attn = outputs.attentions[-1][0, :, -1, generation_config.pos:generation_config.pos_end].mean(dim=0).to(torch.float32).to(final_logits.device).detach().cpu().clone()
                ...
                for layer_id in candidate_premature_layers:
                    ...
                    attn_last_token = attn[0, :, -1, generation_config.pos:generation_config.pos_end].mean(dim=0).to(torch.float32).to(final_logits.device)  # shape: (batch, heads, seq_len)  # batchsize=1的时候
