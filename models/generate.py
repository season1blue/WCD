...
import ipdb
class MyGenerationMixin(ContinuousMixin):
    """
    ...
    def _extract_generation_mode_kwargs(
        ...
        if isinstance(custom_generate, Callable):
            usual_mode_kwargs = inspect.signature(MyGenerationMixin._sample).parameters.keys()
    ...
    def generate(
        ...
        **kwargs,
        ...
        if deprecated_mode_repo := self._get_deprecated_gen_repo(generation_mode, trust_remote_code, custom_generate):
            return MyGenerationMixin.generate(
                self,
                inputs=inputs,
                generation_config=generation_config,
                logits_processor=logits_processor,
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if is_prefill:
                outputs = self(**model_inputs, mask_config=mask_config, return_dict=True)
                is_prefill = False
            else:
                outputs = model_forward(**model_inputs, mask_config=mask_config, return_dict=True)
